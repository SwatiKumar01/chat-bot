...
//                    JSONArray objects = JSON.parseArray(body);
                    if (jsonArray != null) {
                        ...
                        boolean translitPresentInResponse = false;
                        if(translations.size() > 0){
                            JsonArray lastArray = translations.get(translations.size()-1).getAsJsonArray();
                            if(lastArray.size() > 0 && lastArray.get(0).isJsonNull()){
                                //translit is present
                                translitPresentInResponse = true;
                            }
                        }

                        if (translations != null && (translations.size() > 1 || translations.size()>=1 && !translitPresentInResponse)) {
                            if(translitPresentInResponse){
                                //translit is present
                                ...
                                //translit is not present
                                data.setSentences(new ArrayList<TranslateResult.Sentence>(translations.size()));
                                List<TranslateResult.Sentence> sentences = data.getSentences();
